    ...
    (async () => {
      ...
      for (const tag of tags) {
        for (let i = 0; i < 3 * 100; i += step) {
          logger.info(`Admin.update: 类别：${tag}，从 ${i} 到 ${i + step}`);
          const results = await this.douban.suggest(tag as any, i, step)
            .then(v => (logger.info(`Admin.update: 列表获取成功，总计 ${v.length} 项`), v))
            .catch(e => (logger.error(`Admin.update: 列表抓取失败，${tag} 类别从第 ${i} 项到第 ${i + step} 项`), []));
          // 没有更多推荐
          if (results.length === 0) { break; }
          // 循环爬取列表详情
          for await (const result of results) {
            logger.info(`Admin.update: 正在爬取 ${tag} 类别，总计第 ${++this.total} 项`);
            if (await MovieEntity.findOne({ id: result.id })) {
              logger.info(`Admin.update: 数据库中已有数据，跳过 ${result.title}`);
              continue;
            }
            await this.movie.getDetail(result.id)
              .then(v => logger.info(`Admin.update: ${v?.title} 爬取完成，已写入数据库`))
              .catch(e => logger.error(`Admin.update: ${result.id} ${result.title} 爬取失败，原因：${e.message}`));
